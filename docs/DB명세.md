...
## 0-1. 이 문서가 **확인한 것**과 **가정한 것**

솔직하게 갈라 적습니다. 저는 옮겨 갈 서버를 한 번도 못 봤습니다.

### 확인한 것 (기계가 매번 검사합니다)

| | 어떻게 |
|---|---|
| 아래 DDL 의 쉼표·괄호가 맞는지 | 문서를 만들 때마다 훑습니다. 어긋나면 문서를 아예 안 씁니다 |
| 화면별 쿼리가 실제로 도는지 | 문서에 싣기 전에 한 번씩 실행해 봅니다 |
| 드리는 PHP 파일이 5.1 문법인지 | `node scripts/check-php51.js` 로 확인합니다 |
| 연동 토큰 예제가 맞는지 | 그 방식으로 만든 토큰이 통과하는지 테스트가 있습니다 |

### 가정한 것 (틀릴 수 있습니다)

| 가정 | 왜 이렇게 뒀나 | 다르면 |
|---|---|---|
| MySQL **5.0** 대 | PHP 5.1 서버는 대개 그렇습니다 | 더 높으면 utf8mb4 를 쓰시는 게 낫습니다 |
| 기존 회원 표에 칸을 더할 수 있음 | 포인트·캐릭터를 매달 곳이 필요합니다 | 별도 표로 빼야 하면 알려 주세요 |
| 글 · 댓글 표가 이미 있음 | 커뮤니티가 이미 있다고 하셔서 | 없으면 2장의 DDL 을 그대로 쓰시면 됩니다 |
| 회원 id 가 정수 | 우리 쪽이 그렇습니다 | 문자열이면 외래키 타입을 맞춰야 합니다 |

### ⚠ 쿼리는 **SQLite 에서** 돌려 본 것입니다

4장의 쿼리는 지금 구현(SQLite)에서 한 번씩 실행해 확인한 것이고,
MySQL 문법으로는 **손으로 옮겨 적었습니다.** MySQL 에서 직접 돌려 보지는 못했습니다.
날짜 함수처럼 바뀌는 부분은 표로 정리해 뒀지만, 옮기신 뒤 한 번씩 돌려 봐 주세요.

### 서버를 알려 주시면 가정이 사라집니다

`docs/check-server.php` 를 웹 폴더에 올리고 브라우저로 열면
PHP·MySQL 버전, 문자셋, 시간대, 필요한 확장이 있는지, 회원 표에 어떤 칸이
없는지를 한 화면에 보여 줍니다. 그 결과를 주시면 위 가정을 실제 값으로 바꾸겠습니다.
(파일 자체가 PHP 4 문법으로만 쓰여 있어 아주 옛 서버에서도 돕니다.
 **확인 후에는 반드시 지워 주세요** — 서버 정보가 드러납니다.)
...
| 언제 | reason | 금액 | 몇 번까지 · 무엇으로 막는가 |
|---|---|---:|---|
| 커뮤니티 첫 방문(가입) | `signup` | 1,000P | 최초 1회 — 회원을 만들 때 한 번만 부른다 (따로 확인하지 않는다) |
| 출석체크 버튼 | `attendance` | 10P | 하루 1회 — attendance 표의 (user_id, day) UNIQUE 가 막는다 |
| 글 등록 (일반) | `post` | 300P | 하루 3개 — 오늘 point_logs 를 세어서 판정 |
| 글 등록 (익명) | `anon_post` | 100P | 하루 3개 — 오늘 point_logs 를 세어서 판정 |
| 댓글·대댓글 등록 | `comment` | 100P | 하루 10개 — 오늘 point_logs 를 세어서 판정 |
| 내 글이 추천받음 | `like_received` | 10P | 제한 없음 (추천 1개당) |
| 내 글이 인기글이 됨 (추천 10개) | `popular` | 1,000P | 글당 1회 — posts.is_popular 를 세워 두 번 안 준다 |
| 내 글이 운영자 추천글이 됨 | `admin_pick` | 1,500P | 글당 1회 — posts.admin_picked 를 세워 두 번 안 준다 |
| 연속 출석 7일 | `streak7` | 50P | 연속 일수가 **딱 그 날짜일 때만** — 31일째에 또 주지 않는다 |
| 연속 출석 14일 | `streak14` | 100P | 연속 일수가 **딱 그 날짜일 때만** — 31일째에 또 주지 않는다 |
| 연속 출석 21일 | `streak21` | 150P | 연속 일수가 **딱 그 날짜일 때만** — 31일째에 또 주지 않는다 |
| 연속 출석 28일 | `streak28` | 200P | 연속 일수가 **딱 그 날짜일 때만** — 31일째에 또 주지 않는다 |
| 연속 출석 30일 달성 | `streak30` | 100P | 연속 일수가 **딱 그 날짜일 때만** — 31일째에 또 주지 않는다 |
