...
//   ② 고리 안쪽이 얼굴에 딱 붙는다 : FACE = 0.72
// 이러면 칸 밖으로 삐져나가지 않아 목록에서 글 제목을 밀지도 않는다.
//
// ②의 값은 계산이 아니라 **화면에 그려진 것을 재서** 정했다 (scripts/check-ring.js).
// PNG 의 알파값으로 계산하면 0.70 이 나오는데, 고리 안쪽 가장자리는 부드럽게 흐려져서
// 알파가 남아 있어도 눈에는 안 보인다. 그래서 계산상 딱 맞는데 화면에는 흰 띠가 남았다.
// 실제로 그려서 재니 고리 안쪽은 0.72 자리에 앉았다.
//
...
const FACE = 0.72;   // 얼굴을 칸의 몇 배로 그릴지 (화면에서 재서 정한 값)
