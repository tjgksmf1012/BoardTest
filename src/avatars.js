...
//
// 그래서 세 값을 이렇게 잡는다.
//   ① 고리 구멍을 얼굴 한가운데로 밀어 준다 → RING_OFF_X · RING_OFF_Y  ← 이게 진짜 원인이었다
//   ② 고리가 얼굴의 자른 자국 위에 얹힌다   → RING = 1.26
//   ③ 캐릭터 원의 테두리가 고리 몸통 위에 온다 → FACE = 0.92
//
// ②는 세 번 고쳤다. 지나온 값과 그때 들은 말은 이렇다.
//
//   1.20  "캐릭터가 테두리 밖으로 삐져나온다"
//   1.34  "테두리가 아직도 안 맞는다"
//   1.26  지금 값
//
// 세 번 다 크기만 만졌다. 그런데 **크기는 원인이 아니었다.**
// 하늘 님이 "이미지 위치가 프로필 사진이랑 가운데정렬이 잘 안 되어 있을 가능성" 을
// 짚어 주셔서 재 봤더니, 고리 그림 9종 전부 구멍이 오른쪽 위로 치우쳐 있었다(①).
// 한쪽이 뜨면 반대쪽이 삐지는 게 당연했다 — 크기를 어떻게 잡아도 못 고친다.
//
// 재는 방법도 두 번 틀렸다. 처음에는 바깥선의 가운뎃값(0.816)으로 1/0.816 = 1.20,
// 다음에는 최솟값(0.691)으로 0.92/0.691 = 1.34. 계산은 맞는데 화면은 계속 틀렸다.
// 이 고리는 실이 여러 가닥이라 '안쪽 ~ 바깥' 사이가 꽉 차 있지 않다.
// 그래서 '얼굴 가장자리가 그 사이에 있다' 가 '가려진다' 를 뜻하지 않는다.
//
// 제대로 된 물음은 하나다 — **얼굴을 동그랗게 자른 자국 자리에 금색이 있는가.**
// 그 자리(±3px)를 각도 360개에서 세고, 안 가려진 각도를 두 갈래로 나눈다.
// 두 탈은 서로 반대라 동시에 0 이 될 수 없어서 **나쁜 쪽이 가장 작은** 값을 고른다.
// 9종 전부에서 재면 (숫자는 가장 나쁜 그림 기준):
//
//   고리    밀기 전 나쁜 쪽    밀고 나서 나쁜 쪽
//   120%         117                143
//   124%          78                 41
//   126%          72                 13    ← 지금
//   128%         109                 18
//   134%         140                 38    ← "안 맞는다"
//
// 밀어 주는 것만으로 126% 의 나쁜 쪽이 72 → 13 으로, 가려진 각도가 265 → 355 로 갔다.
// 크기는 그대로 두고 위치만 고친 결과다.
//
// 크기를 고를 때도 한 번 헛짚었다. 금색 고리 하나만 놓고 재서 1.28 로 정했는데,
// 파랑·빨강 고리에서는 크게 떠 버렸다. **그림 한 장으로 정하면 안 된다.**
...
// 고리 구멍이 그림 한가운데에 있지 않다 (그림 크기 대비 %).
//
// 크기를 세 번 바꿔도 계속 한쪽이 어긋나서, 하늘 님이 "이미지 위치가 프로필 사진이랑
// 가운데정렬이 잘 안 되어 있을 가능성" 을 짚어 주셨다. 재 보니 그 말이 맞았다.
// 그림 9종 전부 구멍이 오른쪽 위로 같은 만큼 치우쳐 있다 (흩어짐 0.17% · 0.08%).
// 그림을 가운데 놓는 것은 '그림 칸' 을 가운데 놓는 것이지 '구멍' 을 놓는 게 아니다.
// 그래서 오른쪽 위가 뜨고 반대쪽인 왼쪽 아래로 얼굴이 삐져나왔다. 크기로는 못 고친다.
//
// 이만큼 반대로 밀면 구멍이 얼굴 한가운데에 온다. 밀기 전과 후 (나쁜 쪽 / 가려짐):
//   밀기 전  126% → 72 / 265
//   밀고 나서 126% → 13 / 355     (360 각도 중)
const RING_OFF_X = 1.09;     // 구멍이 그림 중심보다 오른쪽으로
const RING_OFF_Y = -5.07;    // 구멍이 그림 중심보다 위로
...
function renderAvatar(avatarCode, borderCode, size = 44, opts = {}) {
  ...
  // 고리를 제자리로 밀어 준다. 그냥 가운데 놓으면 '그림 칸' 이 가운데 올 뿐이고,
  // 정작 얼굴을 감싸야 하는 '구멍' 은 그림 안에서 오른쪽 위로 치우쳐 있다.
  const tx = (-50 - RING_OFF_X).toFixed(2);
  const ty = (-50 - RING_OFF_Y).toFixed(2);
  const ring = ringed
    ? `<img class="avatar-ring" src="${url(size <= THUMB_UP_TO ? b.thumb : b.file)}"`
      + ` style="width:${RING * 100}%;height:${RING * 100}%`
      + `;transform:translate(${tx}%,${ty}%)" alt="" loading="lazy">`
    : '';
...
module.exports = {
  ...
  RING, FACE, RING_OFF_X, RING_OFF_Y, RING_HOLE, RING_EDGE, RING_EDGE_MIN, RING_HOLE_MIN,
