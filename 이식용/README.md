...
`install.sql` 은 표(테이블) 만드는 SQL 입니다.

`lib/` 안에 있는 다섯 개가 실제로 쓰시는 PHP 파일입니다. 포인트, 출석, 캐릭터를 담당합니다.

`캐릭터목록.php` 는 캐릭터 246종의 이름과 값이 적힌 목록이고, `img/avatars/` 에 그림 492장이
들어 있습니다. 한 캐릭터당 큰 그림과 작은 그림 두 장씩이라 그렇습니다.

`화면HTML/` 에는 화면 13개를 HTML 파일로 뽑아 뒀습니다. `원본소스/` 는 지금 돌고 있는
프로그램 전체(Node.js)입니다. 이 둘은 아래에서 따로 설명드리겠습니다.

`test/run.php` 는 이 코드가 제대로 도는지 확인하는 파일이고, `서버확인.php` 는 서버의
PHP 버전이나 설정을 한눈에 보여 주는 파일입니다.
...
목록이나 글, 댓글을 어떻게 처리하는지는 `원본소스/src/routes/board.js` 에 있고,
포인트 규칙은 `원본소스/src/points.js`, 캐릭터와 테두리는 `원본소스/src/avatars.js`,
출석이랑 마이페이지, 상점은 `원본소스/src/routes/user.js` 에 있습니다.
화면 틀은 `원본소스/views/` 폴더의 `.ejs` 파일들입니다.
