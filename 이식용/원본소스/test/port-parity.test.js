...
test('연동 비밀키 안내가 코드와 문서에서 같다', () => {
  // 연동가이드에 서버가 내는 경고 문구를 그대로 옮겨 적어 뒀다.
  // 코드에서 문구나 길이를 고치고 문서를 안 고치면, 선배님은 나오지도 않는 경고를
  // 기다리게 된다. 짧은 키를 막지 않고 '경고만' 하기로 정했으니
  // 그 경고가 문서와 어긋나면 안전장치가 통째로 헛돈다.
  const identity = require('../src/identity');
  const server = fs.readFileSync(path.join(__dirname, '..', 'server.js'), 'utf8');
  const 가이드 = 문서('docs/연동가이드.md');

  assert.ok(server.includes('너무 짧습니다'),
    '서버에 짧은 키 경고가 없어졌어요 — 문서만 남으면 안 됩니다');
  assert.match(가이드, /너무 짧습니다/,
    '연동가이드에 경고 문구가 없어요 (로그만 봐서는 아무도 모릅니다)');
  assert.match(가이드, new RegExp(`${identity.SECRET_MIN}자 이상`),
    `연동가이드의 키 길이가 코드와 달라요 (코드는 ${identity.SECRET_MIN}자)`);
  // '막지 않고 경고만 한다' 는 것도 적혀 있어야 한다 — 안 그러면 못 보고 지나간다
  assert.match(가이드, /경고만 하고|로그를 한 번만 확인/,
    '연동가이드에 경고만 한다는 설명이 없어요');
});
