...
test('좋아요 4개부터 베스트, 3개는 안 된다', async () => {
  const jar = makeJar();
  ...
  // 기준이 4개다. 딱 4개(경계)와 하나 모자란 3개를 같이 둔다.
  const four = mk('좋아요 네개짜리');
  const three = mk('좋아요 세개짜리');
  ...
  likers.slice(0, 3).forEach((u) => like.run(three, u));
  ...
  assert.strictEqual(order[0], four, '베스트 댓글이 맨 위에 있어야 한다');
