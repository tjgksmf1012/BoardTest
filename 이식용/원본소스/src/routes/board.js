...
/* 베스트댓글 기준.
 *
 * 좋아요 수 — 하늘 님이 정하신 값이다. 좋아요 4개부터 베스트로 올라간다.
 *
 *   처음 만들었을 때는 3개였다. 그때 화면에 베스트로 보이던 댓글이 좋아요 4개짜리라
 *   "4개면 올라갔다" 로 기억에 남았고, 그 모습으로 되돌린 것이다.
 *
 *   한동안 5개였다. 선배님 쪽지에 "좋아요 4개 이하는 지정 안 되게" 라고 적혀 있어
 *   글자 그대로 따르면 5부터가 맞았기 때문이다.
 *   지금 값(4)은 그 문장과 어긋난다 — 좋아요가 딱 4개인 댓글이 베스트가 된다.
 *   이 숫자를 다시 건드리기 전에 어느 쪽이 맞는지 한 번 확인하고 고치는 게 좋다.
 *
 * 개수 — 하나만 올린다 (인벤식). 하늘 님이 "베스트 댓글은 하나만" 이라고 하셨다.
 *   처음 만들었을 때는 상위 2개였다(에브리타임식).
 *   기준만 넘으면 전부 올리면, 댓글이 많은 글에서는 절반이 '베스트' 가 되어
 *   뱃지가 아무 뜻도 없어지고 순서도 거의 안 바뀐다.
 *   하나면 '이 글에서 제일 공감받은 댓글' 이라는 뜻이 분명해진다.
 *
 *   같은 좋아요 수가 여럿이면 먼저 달린 댓글이 이긴다 (id 오름차순).
 *   시간이 지나도 뽑히는 댓글이 안 바뀌게 하려는 것이다 —
 *   순서가 흔들리면 새로고침할 때마다 베댓이 바뀌어 보인다.
 */
const BEST_COMMENT_LIKES = 4;
const BEST_COMMENT_MAX = 1;
