...
WHERE p.is_notice = 0 AND p.is_hidden = 0 AND p.category = ?`,
  ...
  {
    screen: '검색',
    note: '제목·본문에 더해 **닉네임으로도** 찾는다.\n\n'
      + '익명 글은 닉네임으로 안 걸리게 빼야 한다. 안 그러면 닉네임을 검색해서 그 사람 '
      + '익명 글이 나오고, 화면에 "익명" 이라고 적혀 있어도 누가 썼는지 드러난다.\n\n'
      + '닉네임을 조건 안에서 users 와 이어 찾으면 OR 때문에 색인을 못 쓰고 글 표를 통째로 '
      + '훑는다 (글 2만 건에서 0.1ms → 7ms). 그래서 걸리는 사람을 **먼저** 찾아 두고 '
      + '`user_id IN (...)` 로 잇는다. 걸리는 사람이 없으면 그 줄을 아예 빼면 된다.\n\n'
      + '두 글자씩 잘라 만든 색인은 옛날 MySQL 에 없어서 아래는 LIKE 로 적었다. '
      + 'MySQL 5.7 이상이면 제목·본문 쪽만 `FULLTEXT ... WITH PARSER ngram` 으로 바꾸면 된다.',
    sql: `-- 1) 닉네임이 걸리는 사람을 먼저 찾는다 (회원 표는 글 표보다 작아서 싸다)
